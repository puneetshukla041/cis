  ...
  return (
    <div className="space-y-6">
      <div className="card p-6">
        <p className="text-sm font-black uppercase text-muted">Manage Test</p>
        <h1 className="mt-1 text-3xl font-black">{test.title}</h1>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <Link href="/tests" className="rounded-xl border px-4 py-2 font-black" style={{ borderColor: "var(--border)" }}>Back</Link>
          <Link href="/upload" className="rounded-xl bg-blue-600 px-4 py-2 font-black text-white">Upload New</Link>
        </div>
      </div>
      ...
      <section className="card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-black">Question Preview</h2>
            <p className="text-sm text-muted">Questions, options, answers and explanations are visible here after upload.</p>
          </div>
          <span className="rounded-full bg-blue-100 px-3 py-1 text-sm font-black text-blue-700">{questions.length} Q</span>
        </div>
        <div className="mt-4 max-h-[680px] space-y-4 overflow-auto pr-1">
          {questions.map((q: any) => (
            <div key={q._id} className="mini-card p-4">
              <p className="font-black leading-7">{q.order}. {q.question}</p>
              <div className="mt-3 grid gap-2 text-sm md:grid-cols-2">
                {(["A", "B", "C", "D"] as const).map((key) => (
                  <p key={key} className={`rounded-xl border px-3 py-2 ${q.answer === key ? "border-green-300 bg-green-50 font-black text-green-800 dark:border-green-900/60 dark:bg-green-950/30 dark:text-green-200" : ""}`} style={{ borderColor: q.answer === key ? undefined : "var(--border)" }}>
                    {key}. {q.options?.[key] || "-"}
                  </p>
                ))}
              </div>
