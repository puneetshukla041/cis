...
export async function POST(request: Request) {
  ...
  try {
    await connectDB();
    const userId = process.env.NEXT_PUBLIC_DEFAULT_USER_ID || "default-user";
    const { testId, mode, practiceType } = startAttemptSchema.parse(await request.json());
    ...
    if (!attempt) {
      attempt = await AttemptModel.create({ userId, testId, testTitle: (test as any).title, paper: (test as any).paper, mode: mode || (test as any).mode, practiceType: practiceType || "learning", durationMinutes: (test as any).durationMinutes, totalQuestions: questionsCount });
    }
    return ok({ attempt });
  } catch (error) {
