...
export default function StatCard({ label, value, helper, tone = "slate" }: Props) {
  ...
  return <div className="card p-5 animate-in"><p className="text-sm font-semibold text-muted">{label}</p><p className="mt-2 text-3xl font-black" style={{color}}>{value}</p>{helper ? <p className="mt-1 text-xs text-muted">{helper}</p> : null}</div>;
}
